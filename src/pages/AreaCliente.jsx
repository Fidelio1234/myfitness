...
export default function AreaCliente() {
  ...
  const [timerAttivo, setTimerAttivo] = useState({});
  ...
  // Sblocca audio al tap (iOS richiede che play() venga chiamato dentro un gesto utente)
  function sbloccaAudio() {
    if (!audioRef.current) {
      audioRef.current = new Audio(BEEP_SRC);
    }
    // Play + pause immediato: questo "sblocca" l'audio su iOS
  ...
  function resetTimer(index, secondi) {
    if (intervalRef.current[index]) clearInterval(intervalRef.current[index]);
    setTimerAttivo(prev => ({ ...prev, [index]: false }));
    setTimer(prev => ({ ...prev, [index]: parseInt(secondi) > 0 ? parseInt(secondi) : 60 }));
  }
      ...
      {scheda && (
        <>
          <div style={styles.tabBar}>
            <button style={{ ...styles.tabBtn, ...(tab === "oggi" ? styles.tabBtnActive : {}) }} onClick={() => setTab("oggi")}>🏋️ Oggi</button>
            <button style={{ ...styles.tabBtn, ...(tab === "storico" ? styles.tabBtnActive : {}) }} onClick={() => setTab("storico")}>📈 Progressi</button>
          </div>
          ...
          {tab === "storico" && cliente && <StoricoCliente clienteId={cliente.id} />}
        </>
      )}
    </div>
  );
